...
export default async function AdminReferralsPage({
  ...
  const t = await getTranslations("panels.admin.referrals", locale);
  const headers = t<{
    id: string;
    order: string;
    partner: string;
  ...
  const statusOptionsLabels = t<
    Record<"all" | "pending" | "approved" | "rejected" | "paid", string>
  >("statusOptions");
  const statusBadgeLabels = t<
    Record<"pending" | "approved" | "rejected" | "paid", string>
  >("statusBadge");
  ...
  const result = await getAdminReferrals({
    ...
    status: (STATUS_OPTIONS.includes(status as any) ? status : "all") as any,
    page,
    perPage: 20,
  });
  ...
  const formatCurrency = (value: number) =>
    `Rp ${value.toLocaleString(intlLocale)}`;
        ...
        <CardContent className="grid gap-4 pt-6 md:grid-cols-4">
          <form className="grid gap-3 md:col-span-3 md:grid-cols-3" action="/admin/referrals">
            ...
            <div>
              ...
              >
                {(STATUS_OPTIONS as ReadonlyArray<(typeof STATUS_OPTIONS)[number]>).map(
                  (option) => (
                    <option key={option} value={option}>
                      {
                        statusOptionsLabels[
                          option as keyof typeof statusOptionsLabels
                        ]
                      }
                    </option>
                  )
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{headers.commission}</TableHead>
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t("codeLabel").replace("{code}", item.linkCode)}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Badge variant="outline" className="capitalize">
                      {statusBadgeLabels[item.status as keyof typeof statusBadgeLabels] ??
                        item.status}
                    </Badge>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button asChild size="sm" variant="outline">
                      <Link href={`/admin/referrals/${item.id}`}>{t("manage")}</Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))
