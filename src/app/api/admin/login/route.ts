...
const ADMIN_LOGIN_RATE_LIMIT = { limit: 5, windowMs: 60_000 } as const;
...
export async function POST(req: Request) {
  ...
  const rateResult = await withRateLimit(
    { key: `admin:login:${hashedIp}`, ...ADMIN_LOGIN_RATE_LIMIT },
    async () => null,
  );
  ...
  try {
    ...
    logger.error('Gagal memproses login admin', error);
    return NextResponse.json(
